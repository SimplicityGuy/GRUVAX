"""Integration tests for B-02 backend fix — GET /api/locate optional profile_id.
...
Tests (RED before the fix in locate.py, GREEN after):
  - test_omitted_profile_id_with_cookie: no profile_id + valid cookie → 200 or 404 (was 422, B-02)
  ...
  - test_supplied_correct_profile_id: supplied correct profile_id + cookie → 200 or 404 (existing path)

Note on the 200-vs-404 distinction: release_id=1 is present in the synthetic seed and
should return 200. However, the key correctness invariant for the omitted-param path is
"not 422 / not 400" (i.e., the session and profile were resolved correctly). We assert
`status_code in (200, 404)` to avoid brittleness if the seed changes, while keeping the
400/403 assertions exact.
...
async def test_omitted_profile_id_with_cookie(client) -> None:  # type: ignore[no-untyped-def]
    """B-02 RED test: omitted profile_id + valid browse-binding cookie → 200 or 404.
...
async def test_no_cookie_returns_session_unbound(no_cookie_client) -> None:  # type: ignore[no-untyped-def]
    ...
    This validation must be preserved exactly after the B-02 fix: no profile_id + no
...
async def test_supplied_correct_profile_id(client) -> None:  # type: ignore[no-untyped-def]
    """Existing path: supplied correct profile_id + matching cookie → 200 or 404 (no regression)."""
    response = await client.get(
        "/api/locate",
        params={"release_id": COVERED_RELEASE_ID, "profile_id": DEFAULT_PROFILE_UUID},
    )
    assert response.status_code in (200, 404), (
        f"Expected 200 or 404 with correct supplied profile_id, got "
        f"{response.status_code}: {response.text}"
    )

...
# gruvax-5dm: the public cube endpoints are profile-scoped, so like search /
# locate / SSE they need a browse-binding to resolve (D-02 / B-02 — no
# default-profile fallback). "Public" means "no PIN", not "no profile".
...
async def test_cube_contents_unbound_returns_400(unbound_client) -> None:  # type: ignore[no-untyped-def]
    """gruvax-5dm: no binding → 400 session_unbound, never the default profile's cube.

    The endpoint must not fall back to the default profile when it cannot tell who
    is asking — that fallback is what made the unscoped read invisible.
    """
    response = await unbound_client.get("/api/cubes/1/0/0")

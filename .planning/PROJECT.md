...
**Shipped:** v2.1 Resilience + Privacy + UX polish (2026-06-03) on top of v2.0 Multi-User Collections (2026-05-30) and v1.0 MVP (2026-05-26) — see [MILESTONES.md](./MILESTONES.md) for the full breakdown of all three.

**No active milestone.** v2.1 closed with all 15 requirements satisfied and audit `passed` (all 5 non-blocking tech-debt items resolved before archive: DEV-05 live UAT, 06-VALIDATION Nyquist doc, two integration warnings, summary-frontmatter backfill). Remaining forward work: backlog **999.2** (LED party / sound-reactive modes, gated on the hardware milestone) and deferred **AUTH-01** (OAuth2 device-grant → a future milestone). Start the next cycle with `/gsd:new-milestone`.

**v2.1 (shipped 2026-06-03):** Phase 10 (Shelf Fill-Overview) complete (2026-06-03) — **UX-01**: the admin `LocatorHeader` mini-Kallax now shades each cube by occupancy (continuous blue `color-mix` gradient via a `--fill` custom property, token-only; empty cubes in the CUBE-05 dashed/desaturated state), with a tap-to-reveal popover showing bin ID + true record count + percentage, and live reshade on `collection_changed`/`boundary_changed` via the first admin-route SSE consumer (`useAdminCubesInvalidation`). 3/3 must-haves verified; developer UAT (Incognito) passed including the empty-vs-full at-a-glance distinction and a UAT-found percentage fix (popover shows the true % e.g. 263%, shading still clamps at 1.0 per D-03); all 3 code-review warnings resolved (button reset + `:focus-visible`, popover overflow flip, overview aria-label). Milestone-closing phase for v2.1. Phase 9 (Offline + Reconnect UX) complete (2026-06-02). Phase 8 (QR Pairing + Privacy + Recently-Pulled) complete (2026-06-01). Phase 7 complete (2026-06-01) — **AUTH-02 + API-04**: members self-connect via a one-time, 1-hour invite link (owner generates PIN-gated, member pastes their own Discogs PAT on a public `/redeem/:code` page; single-use, uniform-404 to avoid an existence oracle, PAT Fernet-encrypted and never returned). Migration 0012 added `profile_invite_codes`, `profile_collection.first_seen_at`, and stored diff state on `profiles`. The sync swap now computes new-record count + initial-import flag (retry-safe pre-DELETE count, Pitfall-4 ordering) and surfaces them on the `collection_changed` SSE payload, an admin NEW RECORDS/IMPORTED diagnostics row, and a kiosk "N new records" pill. 19/19 must-haves verified; code-review warnings WR-02/03/04/05 fixed. Phase 6 complete (2026-05-31) — closed DATA-01 by scoping every admin boundary **write, validation, and read** query to the resolved profile, with `boundary_changed`/`admin_editing` SSE fan-out now per-profile; proven by an extended two-profile isolation suite. DEV-05 wired the kiosk SSE consumer to react live to `device_revoked`/`device_reassigned`. Two live-kiosk UAT items tracked in `06-HUMAN-UAT.md`.
...
## Shipped Milestone: v2.1 — Resilience + Privacy + UX polish (2026-06-03)

**Status:** ✅ Shipped 2026-06-03 — 5 phases (6–10), 16 plans, 15/15 requirements satisfied, audit `passed`. Archives: [`milestones/v2.1-ROADMAP.md`](./milestones/v2.1-ROADMAP.md), [`milestones/v2.1-REQUIREMENTS.md`](./milestones/v2.1-REQUIREMENTS.md), [`milestones/v2.1-MILESTONE-AUDIT.md`](./milestones/v2.1-MILESTONE-AUDIT.md).

**Goal (achieved):** Harden GRUVAX for real household use — let members connect their own collections without the owner handling tokens, make pairing and offline behavior robust, lock down query/history privacy, and polish the shelf-overview UX — while closing v2.0's carried-forward tech debt.
...
- ✓ **Shelf-overview mini-Kallax fill/occupancy** (UX-01 — promoted from backlog 999.1; `LocatorHeader` mini 4×4 shows per-cube fill from `GET /api/admin/cubes`)
- ✓ **v2.0 tech-debt closure** — DEV-02 SSE-immediate kiosk switch/revoke listeners; `profile_id` in the `write_boundary` WHERE clause (DATA-01)
...
---
*Last updated: 2026-06-03 — v2.1 milestone shipped and archived (5 phases, 16 plans, 15/15 reqs, audit `passed` after closing all 5 tech-debt items: DEV-05 Playwright UAT, 06-VALIDATION Nyquist doc, WARNING-1/WARNING-2, summary-frontmatter backfill). Phantom phase 999.1 removed (delivered as Phase 10). No active milestone — next: backlog 999.2 (LED modes) or AUTH-01 (OAuth2, v2.2) via /gsd:new-milestone. — v2.1 Phase 10 (Shelf Fill-Overview) complete — UX-01: admin LocatorHeader mini-Kallax fill shading (token-only blue color-mix `--fill` gradient; CUBE-05 dashed empty state) + tap-to-reveal popover (bin ID + true record count + %) + live reshade via the first admin-route SSE consumer (useAdminCubesInvalidation on collection_changed + boundary_changed). 3/3 must-haves verified; developer UAT passed (incl. empty-vs-full at-a-glance + a UAT-found true-percentage fix, shading still D-03 clamped); 3/3 code-review warnings resolved; 148 frontend tests green. Milestone-closing phase for v2.1. — v2.1 Phase 9 (Offline + Reconnect UX) complete: OFF-03 SSE per-connection retry: jitter (2–8s, no reconnect storm); OFF-01 SSE-authoritative connectivity store + QueryClient networkMode 'always'; OFF-01/02/04 OfflineBanner + degraded read-only mode + "Back online" toast; gap-closure 09-04 added active ['search'] invalidation on reconnect (SC4) and fixed WR-01 (stable toast onDismiss) + WR-02 (clear toast on disconnect). 4/4 must-haves structurally verified; 0 code-review blockers; 124 frontend + full backend tests green. Phase marked complete with 7 live-kiosk timing UAT items tracked in 09-HUMAN-UAT.md (SC1/SC3 ~timing confirmation). — v2.1 Phase 7 (Member Self-Connect + Collection Diff) complete: AUTH-02 invite-token self-connect (one-time 1h link, member pastes own PAT, single-use/uniform-404, PAT encrypted), API-04 collection diff (migration 0012; new-record count + initial-import flag on SSE + admin diagnostics + kiosk pill); 19/19 must-haves verified, WR-02/03/04/05 fixed. — v2.1 Phase 6 (Safe Boundaries + Live Device Lifecycle) complete: DATA-01 profile-scoping hole closed (write+validation+read), DEV-05 kiosk device-lifecycle SSE wired; 2 live-kiosk UAT items tracked. — v2.1 (Resilience + Privacy + UX polish) started. Scope: AUTH-02 self-connect PAT, DEV-04 QR+PIN pairing, API-04 collection diff, SRCH-09 recently-pulled, OFF-01..04 offline UX, PRIV-01..04 privacy, shelf-overview fill (ex-999.1), v2.0 tech-debt closure (DEV-02, write_boundary scoping). Phase numbering continues from Phase 6. Defining requirements next. — v2.0 Multi-User Collections shipped 2026-05-30 (5 phases, 35 plans, +96k LOC across 516 files, 12/12 active reqs; AUTH-01 deferred to v2.2; audit `tech_debt`). v1.0 MVP shipped 2026-05-26 (10 phases, 50 plans, ~36k LOC, 75/75 in-scope reqs). Archives: [`milestones/v2.0-ROADMAP.md`](./milestones/v2.0-ROADMAP.md), [`milestones/v2.0-REQUIREMENTS.md`](./milestones/v2.0-REQUIREMENTS.md), [`milestones/v2.0-MILESTONE-AUDIT.md`](./milestones/v2.0-MILESTONE-AUDIT.md); v1.0: [`milestones/v1.0-ROADMAP.md`](./milestones/v1.0-ROADMAP.md), [`milestones/v1.0-REQUIREMENTS.md`](./milestones/v1.0-REQUIREMENTS.md). Project history: [`MILESTONES.md`](./MILESTONES.md).*

...
**In scope (P2):**
- `profile_id NOT NULL` migration across the 7 v1 tables (`cube_boundaries`, `segments`, `change_log`, `change_sets`, `settings`, `record_stats`, `ambient_baseline`) + composite-uniqueness updates. (Nullable column + default-profile backfill already shipped in P1 per D-11; P2 tightens to NOT NULL.)
